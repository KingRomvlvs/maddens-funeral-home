...
import type { Product, MaterialPreset, InteriorPreset } from "@/lib/marketplace/types";
import { getMaterialById, getInteriorById } from "@/lib/marketplace";

interface CasketDetailContentProps {
  product: Product;
}

export function CasketDetailContent({ product }: CasketDetailContentProps) {
  ...
  const [selectedInterior, setSelectedInterior] = React.useState<
    InteriorPreset | undefined
  >(
    product.defaultInteriorId
      ? getInteriorById(product.defaultInteriorId)
      : product.interiors?.[0]
  );
            ...
            {product.dimensions && (
              <div>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    <p className="text-muted-foreground font-light">Length</p>
                    <p className="font-light">{product.dimensions.length}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground font-light">Width</p>
